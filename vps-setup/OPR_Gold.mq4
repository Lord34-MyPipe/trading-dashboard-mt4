//+------------------------------------------------------------------+
//|                                                     OPR_Gold.mq4 |
//|                  Opening Range Breakout - XAU/USD - MetaTrader 4 |
//|                                                            v1.00 |
//+------------------------------------------------------------------+
...
//  IDENTITE INSTRUMENT
//============================================================
#define EA_NAME        "OPR_Gold"
#define EA_VERSION     "1.00"
#define EA_INSTRUMENT  "XAU/USD"

// Sur l'or : 1 pip = 0.10 USD de mouvement de prix,
// peu importe le nombre de decimales (2 ou 3) du broker.
const double PIP_SIZE_PRICE = 0.10;
...
// --- Bornes du range OPR (en pips pour Gold) ---
extern double MinRangeSize        = 200;
extern double MaxRangeSize        = 600;
...
// --- Identifiants ---
extern int    MagicNumber         = 20250101;
...
double   g_oprLow             = 0.0;
double   g_rangeSizeUnits     = 0.0;   // taille du range en pips/points utilisateur
double   g_atrM5AtEntry       = 0.0;

bool     g_tradeTakenToday    = false;
bool     g_eodClosed          = false;
bool     g_tp1Filled          = false;
int      g_currentDir         = 0;     // +1 long, -1 short
int      g_ticketTP1          = -1;
int      g_ticketTrail        = -1;
...
void OnTick()
{
   // Detection nouveau jour : reset complet de l'etat
   ...
   // Construction / mise a jour du range OPR
   BuildOPRRange();

   // Gestion des positions deja ouvertes
   ManageTrailingStop();

   // Fermeture forcee en fin de session
   CheckEODClose();

   // Recherche d'une entree si pas encore de trade aujourd'hui
...
void EvaluateEntry()
{
   ...
   // Detection nouvelle bougie M5 fermee
   ...
   // Verification des conditions d'entree sur la M5 fermee (index 1)
   int    dir  = 0;
   double slPx = 0.0;
...
bool IsNewsDay()
{
   ...
   bool match = (today.year == news.year && today.mon == news.mon && today.day == news.day);
   return match;
}
...
void BuildOPRRange()
{
   ...
   // Phase de construction : suivi live high/low pendant la fenetre
   if(now < windowEnd)
   {
   ...
   // Fenetre terminee : finalisation via les bougies M1 de la session
   double h = -1.0e15;
   double l =  1.0e15;
   int    barsScanned = 0;
   for(int i = 0; i < 300; i++) // marge large : 300 M1 bars
   {
   ...
   Print(EA_NAME, " Range OPR construit | High=", DoubleToStr(g_oprHigh, _Digits),
         ...
         " Size=", DoubleToStr(g_rangeSizeUnits, 1), " pips");
}
...
bool CheckEntryConditions(int &direction, double &slPrice)
{
   ...
   // 1. Filtre ATR H1 : volatilite du jour > moyenne
   if(FilterATR)
   {
   ...
   // 2 & 3. Verification de la M5 fermee (index 1)
...
double NormalizeLotDown(double lot, double step, double minL, double maxL)
{
   ...
   if(n > maxL) n = maxL;
   // Eviter le bruit de virgule flottante
   int decimals = 2;
...
bool OpenTrade(int dir, double slPrice)
{
   ...
   if(!scaleOut)
   {
      // Pas assez de volume pour split : un seul ordre avec TP1
...
int SendWithRetry(int op, double lot, double price, double sl, double tp, string cmt)
{
   ...
   for(int i = 0; i < MAX_RETRIES; i++)
   {
      ...
      // Si le slippage demande est large, on accepte le prix courant
...
void RecoverState()
{
   // Au cas ou l'EA est attache avec des positions existantes
   datetime today = ComputeDayStart();
   for(int i = 0; i < OrdersTotal(); i++)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
...
void DrawDashboard()
{
   ...
   if(g_rangeBuilt || g_oprHigh > 0)
   {
      ...
      s += "Range      : " + DoubleToStr(g_rangeSizeUnits, 1) + " pips" + nl;
...
void LogTrade(int op, double lot1, double lot2, double entry, double sl, double tp1)
{
   ...
   string msg    = ts + " | " + _Symbol + " | " + dirStr +
                   ...
                   " | range=" + DoubleToStr(g_rangeSizeUnits, 1) + " pips" +
